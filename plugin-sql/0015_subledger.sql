...
CREATE TABLE IF NOT EXISTS __NS__.meter_balances (
...
CREATE TABLE IF NOT EXISTS __NS__.meter_events (
  id                uuid PRIMARY KEY,
  company_id        text NOT NULL,
  holder            text NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS meter_events_reference ON __NS__.meter_events (company_id, reference) WHERE reference IS NOT NULL;
CREATE INDEX IF NOT EXISTS meter_events_unposted ON __NS__.meter_events (company_id, status, batch_id);
CREATE INDEX IF NOT EXISTS meter_events_holder ON __NS__.meter_events (company_id, holder, occurred_at);
CREATE INDEX IF NOT EXISTS meter_events_when ON __NS__.meter_events (company_id, occurred_at);

-- One row per aggregation: which window was rolled up, into which transaction.
CREATE TABLE IF NOT EXISTS __NS__.meter_batches (
  id             uuid PRIMARY KEY,
  company_id     text NOT NULL,
...
CREATE INDEX IF NOT EXISTS meter_batches_company ON __NS__.meter_batches (company_id, to_at);
