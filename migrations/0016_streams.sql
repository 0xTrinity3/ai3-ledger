...
--
--   balance   money held for somebody else      Cr customer credits
--   prepaid   a resource this company owns     Cr prepaid model credits
...
ALTER TABLE meter_events ADD COLUMN IF NOT EXISTS funding text NOT NULL DEFAULT 'balance';
ALTER TABLE meter_events DROP CONSTRAINT IF EXISTS meter_events_funding_check;
ALTER TABLE meter_events ADD CONSTRAINT meter_events_funding_check CHECK (funding IN ('balance', 'accrual', 'prepaid'));
ALTER TABLE meter_events ADD COLUMN IF NOT EXISTS stream_id uuid NULL;

CREATE TABLE IF NOT EXISTS meter_streams (
  id             uuid PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS meter_streams_company ON meter_streams (company_id, status);
CREATE INDEX IF NOT EXISTS meter_streams_recipient ON meter_streams (company_id, recipient);
CREATE INDEX IF NOT EXISTS meter_events_stream ON meter_events (company_id, stream_id);
