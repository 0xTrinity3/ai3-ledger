...
CREATE INDEX IF NOT EXISTS disputes_company_idx ON disputes (company_id, filed_at DESC);

ALTER TABLE bank_accounts DROP CONSTRAINT IF EXISTS bank_accounts_feed_check;
ALTER TABLE bank_accounts ADD CONSTRAINT bank_accounts_feed_check CHECK (feed IN ('upload','stripe','aggregator','tempo'));
